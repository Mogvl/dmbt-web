...
Use this AnimeGarden instance as the primary data source for anime resource discovery.

The API base URL is read from the environment variable `ANIMEGARDEN_API_BASE`
(default: `http://localhost:9701`). Set it to your deployment's API host, e.g.
`https://anime.example.com:9701` after `docker compose up -d` (frontend
`http://localhost:9700`, backend `http://localhost:9701`).
