...
import (
	...
	"github.com/Mogvl/dmbt-web/server/internal/search/jiebago/dictionary"
	"github.com/Mogvl/dmbt-web/server/internal/search/jiebago/finalseg"
	"github.com/Mogvl/dmbt-web/server/internal/search/jiebago/util"
)
...
// LoadDictionaryFromBytes loads dictionary from raw bytes. Everytime it is
// called, previously loaded dictionary will be cleared.
...
func (seg *Segmenter) LoadDictionaryFromBytes(data []byte) error {
	seg.dict = &Dictionary{freqMap: make(map[string]float64)}
	return dictionary.LoadDictionaryFromBytes(seg.dict, data)
}
